...
/*** TO BE DONE START ***/
	...
	if(sent_bytes < 0)
		fail_errno("UDP Error: Could not send message");
...
/*** TO BE DONE START ***/
	...
	if (fcntl(ping_socket, F_SETFL, O_NONBLOCK)!=0) fail_errno("Nonblocking failed\n");
...
/*** TO BE DONE START ***/
	...
	if (gai_rv!=0)
		fail_errno("UDP Error: Server address info not available\n");
...
/*** TO BE DONE START ***/
	if(connect(ping_socket, pong_addrinfo->ai_addr, pong_addrinfo->ai_addrlen)==-1)
		fail_errno("UDP Error: Connection failed\n");
...
/*** TO BE DONE START ***/
	...
	if(ask_socket == -1) {
		fail_errno("UDP Error: Could not create TCP socket");
	}
	
	if(connect(ask_socket, server_addrinfo->ai_addr, server_addrinfo->ai_addrlen) == -1) {
		fail_errno("UDP Error: Connecting to server");
	}
...
/** TO BE DONE START ***/
	...
	if (nr==-1){
			fail_errno("UDP Error: Request not sent\n");
		} 
...
/*** TO BE DONE START ***/
	if (!strcmp(answer,"OK")) {
		fail_errno("UDP Error: Answer Failure\n"); 
		}
