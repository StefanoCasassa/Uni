...
/*** TO BE DONE START ***/
	...
	if (gai_rv!=0) fail_errno("PONG Error: UDP socket Getaddrinfo failed\n");

	udp_socket=socket(pong_addrinfo->ai_family, pong_addrinfo->ai_socktype, pong_addrinfo->ai_protocol);
	if (udp_socket==-1) fail_errno("PONG Error: UDP Socket creation failed\n");
	
	bind_rv=bind(udp_socket,pong_addrinfo->ai_addr,pong_addrinfo->ai_addrlen);
	if (bind_rv==-1) fail_errno("PONG Error: UDP Socket Bind failed\n");
