...
void execute_line(const line_t * const l)
{
	...
	for(int a=0; a<l->n_commands; ++a) {
		...
		if (c->in_pathname) { //Standard input
			assert(a == 0);
		...
		if (c->out_pathname) {
			...
			/*** TO BE DONE START ***/
			if (pipe(fds)==-1) fatal_errno("Pipe Failed\n");
				if (fcntl(fds[0],F_SETFD,FD_CLOEXEC)==-1) fatal_errno("First fcntl Failed\n");
				if (fcntl(fds[1],F_SETFD,FD_CLOEXEC)==-1) fatal_errno("Second fcntl Failed\n");
			/*** TO BE DONE END ***/
			curr_stdout = fds[1];
			next_stdin = fds[0];
		}
