...
/*** TO BE DONE START ***/
	...
	if (sent_bytes==-1){
		fail_errno("TCP Error: Sending the message\n");
	}
	if (sent_bytes!=msg_size) {
		fail_errno("TCP Error: Bytes not all sent\n");
	}
...
/*** TO BE DONE START ***/
	...
	if (gai_rv!=0)
		fail_errno("TCP Error: Server address info not available\n");
...
/*** TO BE DONE START ***/
	...
	if (tcp_socket==-1) fail_errno("TCP Error: Creating the socket\n");
	
	if (connect(tcp_socket,server_addrinfo->ai_addr,server_addrinfo->ai_addrlen)==-1){
		fail_errno("TCP Error: Connection failed\n");
	}
/*** TO BE DONE END ***/
...
/*** TO BE DONE START ***/
	...
	if (nr==-1){
		fail_errno("TCP Error: Request not sent\n");
	} 
...
/*** TO BE DONE START ***/
	...
	if (!strcmp(answer,"OK")) {
		fail_errno("TCP Error: Answer Failure\n"); 
		}
