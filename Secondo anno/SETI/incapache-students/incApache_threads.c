...
#ifdef INCaPACHE_8_1
    int client_sockets[MAX_CONNECTIONS]; /* for each connection, its socket FD */  //socket per client
    int no_response_threads[MAX_CONNECTIONS]; /* for each connection, how many response threads */	//Numero di thread per client (risposta)

    pthread_t thread_ids[MAX_THREADS]; //id ogni thread
    int connection_no[MAX_THREADS]; /* connection_no[i] >= 0 means that i-th thread belongs to connection connection_no[i] *///Quale thread appartiene al client
...
int find_unused_thread_idx(int conn_no) //client Ritorna id thread per il client dato senza però settareil connection_no
    {
		int idx = -1;
...
void send_resp_thread(int out_socket, int response_code, int cookie, int is_http1_0, int connection_idx, int new_thread_idx,char *filename, struct stat *stat_p)
	{
